...
// ---------- Icone da marca (recorte "ask." do logotipo oficial) ----------

const LOGO_SOURCE = { hostname: 'askmenu.pt', path: '/logo.svg' };
const ICON_CROP_RATIO = 0.42; // fracao esquerda do logotipo que contem o "ask."
let iconSvgCache = null;

// fallback local caso o site do logotipo esteja inacessivel
const ICON_FALLBACK = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
<rect width="100" height="100" rx="18" fill="#ffffff"/>
<text x="50" y="66" text-anchor="middle" font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="42" fill="#faa74a">ask.</text>
</svg>`;

function buildCroppedIcon(svgText, opts = {}) {
  // dimensoes do svg original (via viewBox ou width/height)
  let w = 0; let h = 0;
  const vb = svgText.match(/viewBox\s*=\s*["']\s*([\d.\-]+)[\s,]+([\d.\-]+)[\s,]+([\d.]+)[\s,]+([\d.]+)/i);
  if (vb) { w = parseFloat(vb[3]); h = parseFloat(vb[4]); }
  if (!w || !h) {
    const wm = svgText.match(/<svg[^>]*\swidth\s*=\s*["']?([\d.]+)/i);
    const hm = svgText.match(/<svg[^>]*\sheight\s*=\s*["']?([\d.]+)/i);
    if (wm && hm) { w = parseFloat(wm[1]); h = parseFloat(hm[1]); }
  }
  if (!w || !h) return null;

  const cropW = w * ICON_CROP_RATIO;

  // fixa as dimensoes do svg interior para o recorte ser estavel
  const inner = svgText.replace(/<svg([^>]*)>/i, (m, attrs) => {
    let a = attrs
  ...
  if (opts.transparent) {
    // recorte justo, fundo transparente (para usar em linha no cabecalho)
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${cropW} ${h}">${inner}</svg>`;
  }

  // versao quadrada com fundo branco arredondado (favicon / avatar)
  const side = Math.max(cropW, h) * 1.14;
  const padY = (side - h) / 2;
  const padX = (side - cropW) / 2;
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${-padX} ${-padY} ${side} ${side}">` +
    `<rect x="${-padX}" y="${-padY}" width="${side}" height="${side}" rx="${side * 0.18}" fill="#ffffff"/>` +
    inner + `</svg>`;
}

function buildBrandMark(svgText) {
  // versao para fundo escuro: recorte "ask." com as partes escuras em branco
  const cropped = buildCroppedIcon(svgText, { transparent: true });
  if (!cropped) return null;
  return cropped
    .replace(/#231f20/gi, '#ffffff')
    .replace(/#231F20/g, '#ffffff');
}

let rawLogoCache = null;

function fetchLogo(cb) {
  if (rawLogoCache) return cb(rawLogoCache);
  const request = https.get(
    { hostname: LOGO_SOURCE.hostname, path: LOGO_SOURCE.path, timeout: 5000 },
    (resp) => {
      if (resp.statusCode !== 200) { resp.resume(); return cb(null); }
      let data = '';
      resp.on('data', (c) => { data += c; if (data.length > 512 * 1024) resp.destroy(); });
      resp.on('end', () => { rawLogoCache = data; cb(data); });
    }
  );
  request.on('timeout', () => request.destroy());
  request.on('error', () => cb(null));
}

function serveSvg(res, svg) {
  res.writeHead(200, {
...
function handleIcon(req, res) {
  if (iconSvgCache) return serveSvg(res, iconSvgCache);
  fetchLogo((data) => {
    const cropped = data ? buildCroppedIcon(data) : null;
    if (cropped) { iconSvgCache = cropped; return serveSvg(res, cropped); }
    serveSvg(res, ICON_FALLBACK);
  });
}

let brandSvgCache = null;

function handleBrand(req, res) {
  if (brandSvgCache) return serveSvg(res, brandSvgCache);
  fetchLogo((data) => {
    const mark = data ? buildBrandMark(data) : null;
    if (mark) { brandSvgCache = mark; return serveSvg(res, mark); }
    serveSvg(res, `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 76 44"><text x="0" y="33" font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="34" fill="#faa74a">ask</text><text x="62" y="33" font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="34" fill="#ffffff">.</text></svg>`);
  });
}
